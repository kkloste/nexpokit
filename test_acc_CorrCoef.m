...
%
%   Compare corrcoef of top 25 and top 100 ranked nodes vs network size
%
...
for network_id=1:15
    ...
    for trial_number=1:trials
       ...
       %    5 - CC100
        ...
        [~,st] = sort(x, 'descend'); % CC25,CC100_gex
        R = corrcoef(s(1:25),st(1:25));
        record(network_id,4,trial_number) = R(1,2);
        R = corrcoef(s(1:100),st(1:100));
        record(network_id,5,trial_number) = R(1,2);        
       ...
       %    8 - CC100
        ...
        [~,st] = sort(x, 'descend'); % CC25,CC100_expv
        R = corrcoef(s(1:25),st(1:25));
        record(network_id,7,trial_number) = R(1,2);
        R = corrcoef(s(1:100),st(1:100));
        record(network_id,8,trial_number) = R(1,2); 
       ...
       %    11 - CC100
        ...
        [~,st] = sort(x, 'descend'); % CC25,CC100_mexpv
        R = corrcoef(s(1:25),st(1:25));
        record(network_id,10,trial_number) = R(1,2);
        R = corrcoef(s(1:100),st(1:100));
        record(network_id,11,trial_number) = R(1,2); 
